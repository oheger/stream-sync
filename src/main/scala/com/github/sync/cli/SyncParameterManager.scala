...
object SyncParameterManager {
  ...
  /** The name of the option to set the log level for the sync process. */
  final val LogLevelOption = "log-level"
  ...
  /** The help text for the option to set the log level. */
  final val LogLevelHelp =
    """Allows setting a different log level. This controls the amount of output generated by this application. \
      |While per default only warnings and errors are logged, it can be helpful to have more output to \
      |get feedback about the progress of the sync process. Allowed log level are DEBUG, INFO, WARN, and ERROR \
      |(ignoring case).""".stripMargin
  ...
  /**
    * Returns an extractor for the log level option. Here an enum value can be
    * specified, default is the level WARN.
    *
    * @return the extractor for the log level
    */
  private def logLevelExtractor(): CliExtractor[Try[Level]] =
    optionValue(LogLevelOption, Some(LogLevelHelp))
      .toUpper
