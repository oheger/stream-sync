...
object CliActorSystemLifeCycle:
  ...
  Either[ProcessingContext, A] =
    val keyExtractor = ParameterManager.defaultKeyExtractor() andThen { opt =>
      opt.map(key => if key.shortAlias then key else key.copy(key = key.key.toLowerCase(Locale.ROOT)))
    }
...
trait CliActorSystemLifeCycle[C]:
  ...
  /**
    ...
    */
  def execute(args: Seq[String]): Future[String] =
    processCommandLine(args, cliExtractor, helpOptionHelp) match
      case Right(config) =>
