...
import com.github.scli.ParameterExtractor.CliExtractor
...
object CliActorSystemLifeCycle {
  ...
  /**
    ...
    */
  def processCommandLine[A](args: Seq[String], extractor: CliExtractor[Try[A]]): Either[ProcessingContext, A] = {
...
trait CliActorSystemLifeCycle[C] {
  ...
  def execute(args: Array[String]): Future[String] = {
    processCommandLine(args, cliExtractor) match {
      case Right(config) =>
